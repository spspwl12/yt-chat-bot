...
void phash_compute(const uint8_t* gray, int w, int h, uint8_t* out) {
    ...
    /* 3) 2D DCT (행 → 열 순서) */
    /* [수정] row_dct 를 RH * LF 크기로 할당하고 y * LF + u 간격으로 저장.
     *        이전에는 RH * PHASH_DCT_SIZE 로 할당하고 y * PHASH_DCT_SIZE + u 에 저장했으나
     *        LF 개만 채우고 나머지는 0 → 열 방향 패스에서 같은 stride 로 읽으면
     *        저장된 값을 올바르게 참조하지 못하는 버그가 있었음. */
