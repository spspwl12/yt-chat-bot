...
const COMMAND_GROUPS = {
    ...
    'episode': ['!대사', '!몇회', '!몇화', '!몆화', '!몆회', '!몇편', '!편수', '!화차', '!현재회차', '!현재몇편', '!현재몇화', '!지금몇화', '!지금몇회', '!지금몇편', '!지금회차', '!회차', '!ㅁㅎ'],
    'music': ['!음악', '!노래', '!브금', '!곡', '!사운드', '!bgm'],
    'timetable': ['!시간표', '!편성표', '!방영표', '!방송표', '!상영표', '!스케줄', '!스케쥴', '!스캐쥴', '!스캐줄', '!편성', '!순서'],
...
async function handleCommand(type, text, displayName, _input) {
    ...
    // 유저 스탯/시청시간/출석 조회
    if (group === 'stats') {
        return _emitLog(handleStatsCommand(cmd, args, displayName, _input));
    }

    return null;
}

function buildRankMessage(title, rows, valExtractor, cmd, attempt = 0) {
    ...
    const cooldownMsg = getCooldownMsg(cmd);
...
function handleStatsCommand(cmd, args, displayName, _input) {
    ...
    if (rawArg === '총시간' || rawArg === '전체시간') {
        ...
        return {
            msg: buildRankMessage('총 시청시간 순위', rows, (r) => statsTracker.formatWatchTime(r.total_watch_seconds), cmd, 0),
            proc: (attempt) => buildRankMessage('총 시청시간 순위', rows, (r) => statsTracker.formatWatchTime(r.total_watch_seconds), cmd, attempt)
        };
    }
    ...
    if (rawArg === '총채팅' || rawArg === '전체채팅') {
        ...
        return {
            msg: buildRankMessage('총 채팅 순위', rows, (r) => `${r.total_messages.toLocaleString('ko-KR')}개`, cmd, 0),
            proc: (attempt) => buildRankMessage('총 채팅 순위', rows, (r) => `${r.total_messages.toLocaleString('ko-KR')}개`, cmd, attempt)
        };
    }
    ...
    if (rawArg === '오늘채팅' || rawArg === '채팅') {
        ...
        return {
            msg: buildRankMessage('오늘 채팅 순위', rows, (r) => `${r.message_count.toLocaleString('ko-KR')}개`, cmd, 0),
            proc: (attempt) => buildRankMessage('오늘 채팅 순위', rows, (r) => `${r.message_count.toLocaleString('ko-KR')}개`, cmd, attempt)
        };
    }
    ...
    if (rawArg === '오늘시간' || rawArg === '시간') {
        ...
        return {
            msg: buildRankMessage('오늘 시청시간 순위', rows, (r) => statsTracker.formatWatchTime(r.watch_seconds), cmd, 0),
            proc: (attempt) => buildRankMessage('오늘 시청시간 순위', rows, (r) => statsTracker.formatWatchTime(r.watch_seconds), cmd, attempt)
        };
    }

    // 5개 인자 이외의 입력 시 경고
    return returnWarning(msg.stats.invalid_arg, cmd, _input);
}
